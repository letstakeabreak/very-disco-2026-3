...
오디오 에셋은 CC0 녹음만 쓴다(PRD 1.2.1). `assets/source/audio/sources.json`에 원본 페이지·작성자·라이선스·sha256을, `make-audio.py`에 자른 구간을 남기고 `public/assets/audio`에 모노 MP3로 둔다. loader 실패 시 개발 중에는 명시적인 placeholder와 오류 상태를 제공하고, 최종 제출에서는 누락을 성공으로 숨기지 않는다.
