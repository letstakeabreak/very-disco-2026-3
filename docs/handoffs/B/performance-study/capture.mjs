...
const paths = ['docs/handoffs/B/preview.ts', 'docs/handoffs/B/preview.css',
...
const report = { at: new Date().toISOString(), baselineCommit: 'fa622b70566cdacc37cc419552b20ae62b8d61ed',
  kind: 'Desktop Chromium WebGL canvas comparison and draw counts. Neither physical iPhone FPS nor integrated gameplay.', variants: [] };
let session;
const run = (...args) => {
...
try {
  for (const [label, directory, url] of [['before', baselineRoot, baselineUrl], ['after', root, candidateUrl]]) {
    session = `deep-press-shadow-${label}`;
  ...
  report.passed = report.comparisons.every(item => item.identicalPng && item.afterDrawCalls < item.beforeDrawCalls && item.afterTriangles < item.beforeTriangles);
