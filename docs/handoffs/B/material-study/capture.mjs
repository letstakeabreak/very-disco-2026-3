...
const session='deep-press-material-study';
...
const paths=['public/assets/models/press-chamber.glb','public/assets/models/salvage-cassette.glb','src/render/gauge.ts','docs/handoffs/B/material-study/inspector.ts'];
...
try{
 for(const [asset,variant]of [['press','baseline'],['press','reflection'],['cassette','baseline'],['cassette','reflection'],['cassette','amber'],['cassette','amber-opaque']]){
  run('set','viewport','1000','1000');
  const url=`${base}/docs/handoffs/B/material-study/capture.html?asset=${asset}&variant=${variant}`;
  run('open',url);run('wait','--load','networkidle');
  const state=run('eval',`(async()=>{const deadline=performance.now()+20000;while(performance.now()<deadline){const p=window.__materialProbe;if(p?.errors.length)throw new Error(p.errors.join(';'));if(p?.ready){await new Promise(r=>setTimeout(r,300));return {ready:p.ready,errors:p.errors,status:p.status,frames:p.frames,renderer:document.querySelector('canvas').getContext('webgl2').getParameter(7937)};}await new Promise(r=>requestAnimationFrame(r));}throw new Error('timeout');})()`).result;
  ...
  report.captures.push({asset,variant,url,screenshot,sha256:sha(`docs/handoffs/B/material-study/${screenshot}`),...state,console:logs});
