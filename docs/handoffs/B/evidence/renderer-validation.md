...
## 2026-09-25 — 카세트 파손·광학 회귀 및 실제 iPhone 측정 (현재)

카세트 광학 `.94`가 렌즈용 `.72`로 덮어써지던 실제 회귀를 테스트에서 재현하고 렌즈에만 기존 갱신을 적용했다. 생성된 유리 표면에 authored 균열·심한 손상의 열린 구멍을 표현한다. [손상 비교](../damage-study/README.md)는 이전15장·현재21장의 실제 GPU 장면이며 물리 파괴나 게임 판정 검증은 아니다.

[기기 검증](../device-acceptance.md)에서 실제 iPhone 16 Pro Max / iOS27.2 / Safari27.2의3분 fixture를 수행했다. 처음32.1801fps/P95 36ms로 미달했다. DPR2의 표면 해상도를 유지하고 투과 중간 영상만 CSS 해상도로 조정한 뒤40.0544fps/P95 29ms/180.005초/7,210구간/렌더 오류0을 관측했다. 해당 실행의P95 기준은 충족하지만60fps 목표·전체게임·iPhone13급·입력·사용자 인지는 미달 또는 미검증이다. 조정 후 첫 로드의 context-loss1회도 보존하며 다음 정상 실행으로 원인이 해결됐다고 판단하지 않는다.

- `npm run check`: frozen23파일·strictTS·17모듈 경계·11파일59테스트·build 통과. 카세트 optical regression1개 추가.
- art11장, 일반 Chromium3크기·8phase·9물건/상태 조합, 손상21장 재촬영. 모든 해당 보고서 source 전후 해시 일치 및 오류0. 정적 갤러리5목표·35판정·16이미지·3보기·390/320 검사 통과.
- 실제 기기 캡처는 전체 해상도 투과7장과 CSS 해상도 투과7장을 구분한다. 사진 생성물이나 데스크톱 모사가 아니다. 측정 중 화면 캡처를 수행하지 않았다.
- `visibleTriangles105,096`, draw33, GLB4개10,885,424bytes. dist12,169,653bytes, JS677,366bytes/gzip174.61kB. Vite500kB 경고가 남는다.
- preview·art·device QA typecheck, B ownership·diff 공백 검사 통과. 공유 계약/API/A/C/의존성 변경 없음.

[이전 desktop3분](desktop-soak-20260924T164805.json)은27fc244의 증거이며 최신 실기기 결과와 혼합하지 않는다. 아래 날짜별 수치는 각 revision의 이력이다. 아트 미달 판정과 `generated-unverified`4개를 유지한다.

## 2026-09-25 — 카세트 내부 축과 투명 깊이 (27fc244 이력)
