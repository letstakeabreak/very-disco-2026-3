...
후속 [표면 방향 수정](normal-study/README.md)은 압착 형상과 PBR 반사가 어긋나던 원인을 고치고 실제 GPU27조건·전후9쌍으로 확인했다. 아래 콘셉트 비교 표와11장 갤러리는 기존 촬영 revision의 기록으로 유지하며, 해당 수정만으로 전체 아트 판정을 통과로 올리지 않는다.

최신 [케이스 접촉 그림자](case-shadow-study/README.md)는 홈을 가로지르던 평면 그림자를 제거하고 입구·안쪽 벽에 음영을 붙인다. 실제 GPU36조건과 전후12장을 확인했으며 케이스 밖 픽셀 변화는0이었다. 작은 화면에서 개선은 미세하고 케이스 자체는 사진 배경이므로 기존05 접촉·그림자 판정을 전체 통과로 바꾸지 않는다.
