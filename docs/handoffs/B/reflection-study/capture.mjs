...
const session='deep-press-reflection-study';
...
const paths=['public/assets/models/press-chamber.glb','public/assets/models/salvage-cassette.glb','src/render/index.ts','src/render/ram.ts','docs/handoffs/B/reflection-study/capture.ts'];
...
try{
 for(const [asset,variant]of [['reflection','baseline'],['reflection','dark-walls'],['reflection','warm-card'],['reflection','balanced']]){
  run('set','viewport','390','844');
  const url=`${base}/docs/handoffs/B/reflection-study/capture.html?variant=${variant}`;
  run('open',url);run('wait','--load','networkidle');
  const state=run('eval',`(async()=>{const deadline=performance.now()+20000;while(performance.now()<deadline){const p=window.__reflectionProbe;if(p?.errors.length)throw new Error(p.errors.join(';'));if(p?.ready){await new Promise(r=>setTimeout(r,300));return {ready:p.ready,errors:p.errors,status:p.status,materials:p.materials,renderer:document.querySelector('canvas').getContext('webgl2').getParameter(7937)};}await new Promise(r=>requestAnimationFrame(r));}throw new Error('timeout');})()`).result;
  ...
  report.captures.push({asset,variant,url,screenshot,sha256:sha(`docs/handoffs/B/reflection-study/${screenshot}`),...state,console:logs});
  console.log(`${asset} ${variant}: ready, ${state.status.drawCalls} visible calls, ${state.status.triangles} reported triangles`);
 }
