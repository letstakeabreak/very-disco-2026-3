...
import type { BufferGeometry, Camera, WebGLProgramParametersWithUniforms } from 'three';
...
let caseContact: Mesh | null = null;
let worktop: Mesh | null = null;
let shapedWorktop: BufferGeometry | null = null;
const oldWorktop = new PlaneGeometry(5, 5);
...
Mesh.prototype.onBeforeRender = function (gpu, scene, view, geometry, material, group): void {
  ...
  if (this.name === 'case-contact-shadow') caseContact = this;
  if (this.name === 'worktop-shadow') { worktop = this; shapedWorktop ??= this.geometry; }
...
function useCaseReceivers(enabled: boolean): void {
  if (!caseContact || !worktop || !shapedWorktop) throw new Error('Missing receiver geometry');
  caseContact.visible = enabled;
  worktop.geometry = enabled ? shapedWorktop : oldWorktop;
  render(lastSnapshot);
}
function compareShadows(order: SalvageId[], compression: number): Record<string, unknown> {
  show(order, compression);
  const gl = canvas.getContext('webgl2')!;
  const pixels = (): Uint8Array => {
    const data = new Uint8Array(canvas.width * canvas.height * 4);
  ...
  // Reproduce the old receiver in the same frame: 5m plane crossing the case,
  // unchanged opaque depth mask, no separate case shadow receiver.
  useCaseReceivers(false); const before = pixels(); const beforeStatus = { ...canvas.dataset };
  useCaseReceivers(true); const after = pixels(); const afterStatus = { ...canvas.dataset };
  const stageH = Math.min(innerHeight, innerWidth * 1.5); const stageW = stageH * 2 / 3;
  let changed = 0; let outside = 0; let darkened = 0; let lightened = 0;
  for (let y = 0; y < canvas.height; y++) for (let x = 0; x < canvas.width; x++) {
    const offset = ((canvas.height - 1 - y) * canvas.width + x) * 4;
    const differences = [0,1,2].map(channel => after[offset+channel]! - before[offset+channel]!);
    if (Math.max(...differences.map(Math.abs)) <= 3) continue;
    changed++;
    const u = (x - (innerWidth-stageW)/2)/stageW*1024;
    const v = (y - (innerHeight-stageH)/2)/stageH*1536;
    if (u < 390 || u > 990 || v < 1045 || v > 1360) outside++;
    if (differences.reduce((a,b)=>a+b,0) < 0) darkened++; else lightened++;
  }
  return { changedPixels: changed, changedOutsideCase: outside, darkenedPixels: darkened, lightenedPixels: lightened,
    beforeStatus, afterStatus, readback: 'Same-production-renderer synchronous WebGL2 readPixels, RGB threshold >3/255; old plane/no receiver versus case footprint hole and case receiver.', errors };
}
let frame = 0;
const load = (): void => {
...
Object.assign(window, { __caseProbe: { show, compareDepth, compareShadows, useCaseReceivers, errors, get ready() { return canvas.dataset['renderState'] === 'ready'; } } });
window.addEventListener('error', event => errors.push(event.message));
window.addEventListener('unhandledrejection', event => errors.push(String(event.reason)));
const dispose = (): void => { cancelAnimationFrame(frame); window.removeEventListener('resize', resize); if(worktop && shapedWorktop) worktop.geometry=shapedWorktop; renderer.dispose(); oldWorktop.dispose(); Mesh.prototype.onBeforeRender = original; };
