...
const run = (...args) => { const r = JSON.parse(execFileSync('npx', ['--yes', 'agent-browser@0.38.1', '--session', 'deep-press-case-shadow-proof', '--json', ...args], { cwd: root, encoding: 'utf8', timeout: 45000 })); if (!r.success) throw new Error(JSON.stringify(r.error)); return r.data; };
...
const paths = ['src/render/index.ts', 'src/render/cassette.ts', 'src/render/case-depth.ts', 'src/render/deformation.ts', 'src/render/ram.ts', 'public/assets/textures/workshop-v4.webp',
  ...
  'docs/handoffs/B/case-shadow-study/foreground.ts', 'docs/handoffs/B/case-shadow-study/foreground.html', 'docs/handoffs/B/case-shadow-study/check.mjs'];
const hashes = () => Object.fromEntries(paths.map(p => [p, sha(p)]));
const report = { checkedAt: new Date().toISOString(), kind: 'Actual WebGL production renderer with synthetic stored-history fixtures. CPU projection of actual deformed GLB vertices plus synchronous GPU pixel comparison with depth on/off and empty case. Includes same-frame old/new shadow receiver comparison. Not physical containment, C HUD, gameplay or iPhone validation.', sourceHashesBefore: hashes(), layouts: [], captures: [] };
mkdirSync(directory + 'foreground-captures', { recursive: true });
try {
  for (const [width, height] of [[390, 844], [320, 568]]) {
    run('set', 'viewport', String(width), String(height));
    run('open', `${process.argv[2] ?? 'http://127.0.0.1:5173'}/docs/handoffs/B/case-shadow-study/foreground.html`);
    run('wait', '--load', 'networkidle'); run('snapshot', '-i');
    const layouts = run('eval', `(async()=>{for(let i=0;i<1200&&!window.__caseProbe?.ready;i++)await new Promise(r=>requestAnimationFrame(r));const p=window.__caseProbe;if(!p?.ready)throw new Error('Not ready');const ids=['salvage-cassette','salvage-lens','salvage-core'];const out=[];for(const a of ids)for(const b of ids.filter(x=>x!==a)){const c=ids.find(x=>x!==a&&x!==b);for(const compression of [0,.55,1]){const result=p.show([a,b,c],compression);result.depth=p.compareDepth([a,b,c],compression);result.shadow=p.compareShadows([a,b,c],compression);out.push(result);}}return out;})()`).result;
    for (const layout of layouts) {
      ...
      const shade = layout.shadow;
      if(shade.errors.length || shade.changedOutsideCase !== 0 || shade.changedPixels < 5) throw new Error(`Shadow receiver regression: ${JSON.stringify(shade)}`);
    ...
    for (const compression of [0, .55, 1]) {
      ...
      run('eval', 'window.__caseProbe.useCaseReceivers(false)');
      const beforeScreenshot = `foreground-captures/${width}-compression-${compression}-before.png`;
      run('screenshot', directory + beforeScreenshot);
      run('eval', 'window.__caseProbe.useCaseReceivers(true)');
      const screenshot = `foreground-captures/${width}-compression-${compression}.png`; run('screenshot', directory + screenshot);
      report.captures.push({ state, beforeScreenshot, beforeSha256: sha(`docs/handoffs/B/case-shadow-study/${beforeScreenshot}`),
        screenshot, sha256: sha(`docs/handoffs/B/case-shadow-study/${screenshot}`) });
    }
  }
  report.errors = run('errors').errors ?? []; report.console = run('console');
  if (report.errors.length || (report.console.messages ?? []).some(m => m.type === 'error' || /Multiple instances of Three/.test(m.text??''))) throw new Error('Browser error');
