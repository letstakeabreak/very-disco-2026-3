...
재검사: `node docs/handoffs/B/check-public-access.mjs`. 익명 API/문서 요청, 임시 filtered clone, 공통 지침 읽기, B fetch/문서 읽기를 실행하고 임시 폴더를 정리한다. 전체 에셋 checkout·push·상대 계정 세션은 검사하지 않는다. 예상한 main/B 문서 404도 별도의 진단 항목으로 기록한다. main에 B 문서가 통합되면 그 기대 상태를 200으로 갱신해야 한다.
