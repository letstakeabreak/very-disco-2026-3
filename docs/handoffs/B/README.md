...
직전 보완은 [케이스 접촉 그림자](case-shadow-study/README.md)다. 작업대 그림자 면이 케이스 홈을 가로지르지 않게 하고 폼 입구·안쪽 벽에서 그림자를 받는다. 실제 GPU36조건·전후12장·전체69테스트·build를 통과했다. 정지 자세는 draw28 / 모든 패스 triangles193,272다. 작은 화면의 차이는 미세하며 전체 아트 합격을 뜻하지 않는다.
...
- [선택용 한국인 남녀 캐릭터 2종](../../../assets/source/characters/korean-salvagers-v1/README.md)을 ImageGen 원본·정확한 프롬프트·생성 기록과 함께 보존했다. 심해 작업실의 청록·아이보리·구리·호박색을 공유하는 가상 성인 캐릭터다. 게임과 배포에는 미적용이며 NPC·3D 모델 구현을 뜻하지 않는다.
...
- PBR 재질, 따뜻한 작업등/약한 청백색 보조광, 좁은 환경 반사, 접촉 그림자를 사용한다. [재질 실험](material-study/README.md) 이후 [환경 반사 비교](reflection-study/README.md)의 balanced 설정(실제 Standard 벽 반사색 ×.35, 왼쪽/전방의 따뜻한 카드)을 채택했다. [원본 단면 조사](cassette-structure-study/README.md)에서 중앙 코어가 없음을 확인한 뒤 새 내부 축을 별도 생성했다. 원본의 숨겨진 구조를 복원한 것이 아니다. 현재 카세트는 그 축과 반대쪽 외피가 보이는 투명 패스를 사용하며 유리 경계와 원본 세부 차이는 남는다.
