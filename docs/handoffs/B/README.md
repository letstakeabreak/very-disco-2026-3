...
최신 보완은 램의 접촉 진입 애니메이션이다. [실제 renderer의 프레임별 증거](retraction-study/motion-report.json)와 최신 브라우저 검사의 소스 SHA로 구현을 식별한다. 5개 아트 목표의 정적 비교·환경 반사·원본 형상 조사는 직전 구현 `2c859819946af781fbb25709839b7c4675aa4b3b`의 기록이며 새 연속 동작의 증거와 구분한다. 상태는 **검증 가능한 B 구현 / 아트 미달 항목 및 실기기 미검증이 남은 Draft**다.
