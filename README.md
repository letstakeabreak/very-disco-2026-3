...
심해 회수품을 압착해 1L 케이스에 담는 iPhone 게임. 더 작게 만들수록 많이 회수할 수 있지만 내부 코어의 가치가 떨어진다. 이 저장소는 3인 독립 개발의 PRD·계약·연구·아트 원본과, 그 위에서 A 코어·B 렌더·C 앱을 통합한 **v2 게임**이다(판단 단서, 작업대 UI, 회수 이야기 포함). 확인한 범위와 남은 조건은 [검증 현황](docs/validation.md)에 있다.
