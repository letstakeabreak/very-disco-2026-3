...
**오디오(v1.2.0):** 효과음과 배경음은 C가 Web Audio로 코드에서 합성한다. 음원 파일과 음성은 넣지 않는다. 첫 사용자 입력 뒤에 켜고, 음소거를 제공하며 기기에 기억한다. 접근 가능하지 않은 Meshy나 실기기가 있더라도 독립 모듈 개발은 계속하고 해당 증거를 미완료로 남긴다. AI 에셋 허용 정책·참가 자격·제출용 이름·웹 인정 경로는 대회 확인 목록이며 승인받은 사실로 기록하지 않는다.
